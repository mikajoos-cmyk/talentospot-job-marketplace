...
export const jobAlertService = {
    ...
    async getMatchingJobs(filters: JobFiltersState) {
        ...
        const useClientSideFiltering = filters.enableFlexibleMatch || filters.enablePartialMatch;
        ...
        if (!useClientSideFiltering) {
            ...
            if (filters.contractDuration) searchParams.contract_duration = filters.contractDuration;
            if (filters.benefits && filters.benefits.length > 0) searchParams.benefits = filters.benefits;
        }

        const data = await jobsService.searchJobs(searchParams, filters.workRadius || 200);
        let results = data || [];

        // Apply client-side filtering/scoring if needed
        if (useClientSideFiltering) {
            const { calculateJobMatchScore } = await import('@/utils/match-utils');
            const flexibleMode = filters.enableFlexibleMatch;

            results = results.map(job => ({
                ...job,
                matchScore: calculateJobMatchScore(job, filters, flexibleMode)
            }));

            // Filter by threshold only if partial matching is enabled
            if (filters.enablePartialMatch) {
                results = results.filter(job => (job.matchScore || 0) >= filters.minMatchThreshold);
            }
